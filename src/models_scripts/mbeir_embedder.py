...
import dist_utils
from dist_utils import setup_for_distributed, ContiguousDistributedSampler
from utils import build_model_from_config
from data.mbeir_data_utils import DatasetType, build_mbeir_dataset_from_config
    ...
    for split_name, split_dir, dataset_name_list, cand_pool_name_list in splits:
        for dataset_name, cand_pool_name in zip(dataset_name_list, cand_pool_name_list):
            ...
            if not dist.is_initialized() or dist.get_rank() == 0:
                ...
                embed_path = os.path.join(mbeir_dir, embed_dir_name, expt_dir_name, split_name, embed_data_name)
                ...
                id_path = os.path.join(mbeir_dir, embed_dir_name, expt_dir_name, split_name, id_data_name)
        ...
        if split_name == "cand_pool" and embed_cand_pool_config.embed_union_pool:
            # Concat previously saved embeddings and ids from single pool
            if not dist.is_initialized() or dist.get_rank() == 0:

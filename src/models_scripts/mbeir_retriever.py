...
def create_index(config):
    ...
    for cand_pool_name in cand_pool_name_list:
        ...
        embed_data_path = os.path.join(mbeir_dir, embed_dir_name, expt_dir_name, split_name, embed_data_file)
        embed_data_hashed_id_file = f"mbeir_{cand_pool_name}_{split_name}_ids.npy"
        embed_data_hashed_id_path = os.path.join(
            mbeir_dir, embed_dir_name, expt_dir_name, split_name, embed_data_hashed_id_file
        )
        ...
        index_path = os.path.join(
            mbeir_dir, index_dir_name, expt_dir_name, split_name, f"mbeir_{cand_pool_name}_{split_name}.index"
        )
...
def run_retrieval(config):
    ...
    exp_results_dir = os.path.join(mbeir_dir, results_dir_name, expt_dir_name)
    ...
    for split_name in dataset_types:
        ...
        if retrieval_dataset_config and retrieval_dataset_config.enable_retrieve:
            ...
            dataset_embed_dir = os.path.join(mbeir_dir, embed_dir_name, expt_dir_name, split_name)
    ...
    eval_results = []
    cand_index_dir = os.path.join(mbeir_dir, index_dir_name, expt_dir_name, "cand_pool")
...
def run_hard_negative_mining(config):
    ...
    # Load query IDs
    dataset_embed_dir = os.path.join(mbeir_dir, embed_dir_name, expt_dir_name, dataset_split_name)
    ...
    cand_index_dir = os.path.join(mbeir_dir, index_dir_name, expt_dir_name, cand_pool_split_name)
...
def parse_arguments():
    ...
    parser.add_argument("--mbeir_dir", type=str, default="/data/mbeir")
    parser.add_argument("--mbeir_data_dir", type=str, default="/data/mbeir/mbeir_data")
...
def main():
    ...
    config.mbeir_dir = args.mbeir_dir

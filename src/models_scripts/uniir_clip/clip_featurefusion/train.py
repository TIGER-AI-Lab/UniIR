...
from models_scripts.uniir_clip.engine import train_one_epoch, eval_engine
from models_scripts.uniir_clip import utils
from clip_ff import CLIPFeatureFusion_OPENAI
...
def create_optimizer(gain_or_bias_params, rest_params, t5_params, config):
    return optim.AdamW(
        [
            {"params": gain_or_bias_params, "weight_decay": 0.0},
            {"params": rest_params, "weight_decay": 0.2},
            {"params": t5_params, "weight_decay": 0.2, "lr": config.trainer_config.t5_learning_rate},
        ],
...
def save_checkpoint(model, optimizer, scheduler, epoch, scaler, config):
    ...
    checkpoint_path = os.path.join(config.mbeir_dir, ckpt_config.ckpt_dir, checkpoint_name)
...
def main(config):
    ...
    pretrained_clip_model_dir = os.path.join(config.mbeir_dir, model_config.pretrained_clip_model_dir)
    logger.info(f"Downloading CLIP model to {pretrained_clip_model_dir}...")
    model = CLIPFeatureFusion_OPENAI(
    ...
    t5_params = filter_parameters(model, t5_condition)
    optimizer = create_optimizer(gain_or_bias_params, rest_params, t5_params, config)
    ...
    if ckpt_config.resume_training:
        checkpoint_path = os.path.join(config.mbeir_dir, ckpt_config.ckpt_dir, ckpt_config.ckpt_name)

from models_scripts.uniir_blip.models.med import BertConfig, BertModel
...
from models_scripts.uniir_blip.models.blip import create_vit, init_tokenizer, load_checkpoint
from models_scripts.uniir_blip.transform.blip_transform import get_blip_transform
...
class BLIPScoreFusion(nn.Module):
    ...
    def encode_texts(self, texts: dict, use_momentum=False):
        ...
        if use_momentum:
            text_feat = self.text_encoder_m(
                texts.input_ids, attention_mask=texts.attention_mask, return_dict=True, mode="text"
            ).last_hidden_state[:, 0, :]
    ...
    def encode_multimodal_input(self, txt_dict_batched, image_batched, txt_mask, img_mask, use_momentum=False):
        ...
        Args:
            txt_dict_batched
            img_batch (torch.Tensor): images in shape [batch_size, C, H, W]

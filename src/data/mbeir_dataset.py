...
class MBEIRDatasetBase(Dataset):
    ...
    def _load_cand_instructions(self, instructions_path):
        """Validate and load instructions."""
        full_instructions_path = os.path.join(self.mbeir_data_dir, instructions_path)
        # Validate the path and file extension
        assert os.path.exists(
            full_instructions_path
        ), f"Candidate Instructions Path {full_instructions_path} does not exist"
        assert full_instructions_path.endswith(
            ".tsv"
        ), f"Candidate Instructions Path {full_instructions_path} is not a tsv file"
        prompts_dict = {}
        with open(full_instructions_path, "r") as f:
            for line in f.readlines():
                parts = line.strip().split("\t")
                ...
                prompts_dict[key] = prompts
        self.candidate_instructions = prompts_dict
    ...
    def _get_random_cand_prompt(self, dataset_id, cand_modality):
        ...
        prompts = self.candidate_instructions.get(key, [])
        assert prompts, f"Cannot find prompts for {key}"
        prompt = format_string(random.choice(prompts))
        assert prompt, f"Prompt is empty for {key}"
        return prompt
...
class MBEIRMainDataset(MBEIRDatasetBase):
    def __init__(
        ...
        cand_instruct_path,  # Relate path to the candidate instructions
        img_preprocess_fn,
        mode=Mode.TRAIN,
        enable_query_instruct=True,  # Whether to enable instructions
        enable_cand_instruct=True,  # Whether to enable candidate metadata instructions
        ...
        self._load_cand_instructions(cand_instruct_path)
        ...
        self.enable_cand_instruct = enable_cand_instruct
        ...
        if print_config:
            ...
            self.cand_instruct_path = cand_instruct_path
            self._print_config()
    ...
    def _print_config(self):
        ...
        print(f"Enable Candidate Instructions: {self.enable_cand_instruct}")
        if self.enable_cand_instruct:
            print(f"Candidate Instructions Path: {self.cand_instruct_path}")
    ...
    def __getitem__(self, index):
        ...
        if self.mode == Mode.TRAIN:
            ...
            pos_cand = _prepare_data_dict(
                pos_cand_txt_with_prompt if self.enable_cand_instruct else pos_cand_txt_without_prompt,
            ...
            neg_cand_list = [
                _prepare_data_dict(
                    neg_cand["txt_with_prompt"] if self.enable_cand_instruct else neg_cand["txt_without_prompt"],
...
class MBEIRCandidatePoolDataset(MBEIRDatasetBase):
    def __init__(
        ...
        cand_instruct_path,  # Relate path to the candidate instructions
        img_preprocess_fn,
        enable_cand_instruct=True,  # Whether to enable instructions
        returns=None,  # Catch any return-related settings
        print_config=True,  # Whether to print the dataset config
    ):
        super().__init__(mbeir_data_dir, img_preprocess_fn)
    ...
    def __getitem__(self, index):
        ...
        instance = {
            "txt": cand_txt_with_prompt if self.enable_cand_instruct else cand_txt_without_prompt,
            "img": img,
